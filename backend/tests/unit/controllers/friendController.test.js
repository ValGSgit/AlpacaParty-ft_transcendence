...
describe("listFriends", () => {
  test("returns friends with default pagination", async () => {
    const getFriendsResult = {
      friends: [{ id: 2, username: "bob" }],
      count: 1,
    };
    mockFriend.getFriends.mockResolvedValue(getFriendsResult);
    const { req, res, next } = createReqRes();
    await listFriends(req, res, next);
    expect(mockFriend.getFriends).toHaveBeenCalledWith(1, {
      limit: 50,
      offset: 0,
      filter: undefined,
      sort: undefined,
    });
    expect(res._json.friends).toEqual(getFriendsResult.friends);
  });
...
describe("sendRequest", () => {
  test("sends friend request successfully", async () => {
    ...
    mockFriend.sendRequest.mockResolvedValue({
      id: 1,
      senderId: 1,
      receiverId: 2,
      status: "pending",
    ...
    expect(mockNotificationService.friendRequest).toHaveBeenCalledWith(
      2,
      "tester",
    );
  });
...
describe("acceptRequest", () => {
  test("accepts request successfully", async () => {
    ...
    expect(mockNotificationService.friendAccepted).toHaveBeenCalledWith(
      2,
      "tester",
    );
