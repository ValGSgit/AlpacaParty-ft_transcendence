...
} from "#validators/validatorUtils.js";
...
const { userUpdateValidation } =
  await import("../../../src/validators/userValidator.js");
...
describe("updateMe", () => {
  test("should reject bio longer than 500 characters", async () => {
    ...
    expect(errors.errors[0].msg).toMatch(/bio must be/i);
  });

  test("should reject status longer than 200 characters", async () => {
    const { req, res, next } = createReqRes({
      body: { status: "a".repeat(201) },
    });
    await runUpdateValidation(req);
    const errors = customValidationResult(req);
    expect(errors.errors[0].msg).toMatch(/status must be/i);
  });
  ...
  test("should reject username shorter than 3 characters", async () => {
    ...
    await runUpdateValidation(req);
    const errors = customValidationResult(req);
    expect(errors.errors[0].msg).toMatch(/username must be/i);
  });
  ...
  test("should reject username with invalid characters", async () => {
    ...
    await runUpdateValidation(req);
    const errors = customValidationResult(req);
    expect(errors.errors[0].msg).toMatch(/username may only/i);
  });
  ...
  test("should reject email longer than 254 characters", async () => {
    const { req, res, next } = createReqRes({
      body: { email: "a".repeat(250) + "@b.com" },
    });
    await runUpdateValidation(req);
    const errors = customValidationResult(req);
    expect(errors.errors[0].msg).toMatch(/email must be/i);
  });
...
describe("changePassword", () => {
  ...
  test("should reject wrong current password", async () => {
    mockUser.findByIdWithPassword.mockResolvedValue({
      id: 1,
      passwordHash: "hash",
    });
...
describe("requestDeletion", () => {
  test("should create deletion request", async () => {
    ...
    expect(mockDataRequest.create).toHaveBeenCalledWith({
      userId: 1,
      type: "delete",
    });
  ...
  test("should reject if deletion request already pending", async () => {
    ...
    expect(res._status).toBe(409);
    expect(res._json).toEqual({
      error: { message: "A deletion request is already pending" },
    });
