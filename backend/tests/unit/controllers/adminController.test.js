...
describe('adminController', () => {
  ...
  describe('adminLogin', () => {
    ...
    test('should create token and set cookie on successful login by username', async () => {
      ...
      prisma.user.findUnique.mockResolvedValue(user);
      AuthService.comparePassword.mockResolvedValue(true);
      AdminAuthService.generateToken.mockReturnValue('test-token');
      
      await adminController.adminLogin(req, res, next);
      
      expect(AdminAuthService.generateToken).toHaveBeenCalledWith(user);
      expect(res.cookie).toHaveBeenCalledWith('admin_jwt_token', 'test-token', expect.any(Object));
  ...
  describe('adminLogout', () => {
    test('should clear admin_jwt_token cookie', () => {
      ...
      expect(res.clearCookie).toHaveBeenCalledWith('admin_jwt_token', { path: '/' });
  ...
  describe('getAdminPermissions', () => {
    test('should return admin permissions from Vault', async () => {
      ...
      expect(Vault.read).toHaveBeenCalledWith('secret/data/alpacaparty/admins/1');
  ...
  describe('setAdminPermissions', () => {
    ...
    test('should set admin permissions', async () => {
      ...
      expect(Vault.write).toHaveBeenCalledWith('secret/data/alpacaparty/admins/2', permissions);
      expect(res.json).toHaveBeenCalledWith({ permissions });
    });
  });
});
