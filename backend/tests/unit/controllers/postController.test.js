...
describe('getFeed', () => {
  test('should return posts array', async () => {
    ...
    expect(mockPost.getFeed).toHaveBeenCalledWith({ limit: 20, offset: 0, viewerId: 1 });
  ...
  test('should respect limit and offset query params', async () => {
    ...
    const { req, res, next } = createReqRes({ query: { limit: '5', offset: '10' } });
    await getFeed(req, res, next);

    expect(mockPost.getFeed).toHaveBeenCalledWith({ limit: 5, offset: 10, viewerId: 1 });
  });
...
describe('getUserPosts', () => {
  test('should return posts for a user', async () => {
    ...
    expect(mockPost.getByUser).toHaveBeenCalledWith(42, { limit: 20, offset: 0 });
    expect(res._json).toEqual({ posts });
  });
...
describe('createPost', () => {
  ...
  test('should reject when content exceeds 5000 characters', async () => {
    const { req, res, next } = createReqRes({ body: { content: 'a'.repeat(5001) } });
    await createPost(req, res, next);

    expect(res._status).toBe(400);
    expect(res._json).toEqual({ error: { message: 'content must be 5000 characters or fewer' } });
  });

  test('should reject invalid imageUrl', async () => {
    const { req, res, next } = createReqRes({ body: { content: 'hello', imageUrl: 123 } });
    await createPost(req, res, next);
  ...
  test('should create post successfully', async () => {
    ...
    expect(mockPost.create).toHaveBeenCalledWith({
      authorId: 1, content: 'hello', imageUrl: null, isPublic: true,
    });
...
describe('updatePost', () => {
  test('should reject empty content', async () => {
    ...
    expect(res._status).toBe(400);
    expect(res._json).toEqual({ error: { message: 'content must be between 1 and 5000 characters' } });
  });
  ...
  test('should update post successfully', async () => {
    ...
    expect(mockPost.update).toHaveBeenCalledWith(1, 1, {
      content: 'updated', imageUrl: undefined, isPublic: undefined,
    });
    expect(res._json).toEqual({ post });
  });
...
describe('deletePost', () => {
  ...
  test('should succeed even if delete returns false when user is admin', async () => {
    mockPost.delete.mockResolvedValue(false);
...
describe('likePost', () => {
  ...
  test('should send notification if liking someone elses post', async () => {
    ...
    expect(mockNotificationService.postLiked).toHaveBeenCalledWith(5, 'alice', 10);
  });
