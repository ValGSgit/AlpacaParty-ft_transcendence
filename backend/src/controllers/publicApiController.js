...
export const getLeaderboard = async (req, res, next) => {
  try {
    ...
    const anonymize = ["1", "true", "yes"].includes(
      String(req.query.anonymized || "").toLowerCase(),
    );
...
export const getPosts = async (req, res, next) => {
  try {
    ...
    const shaped = posts.map((p) => ({
      ...
      author_username: anonymize
        ? anonymizeName(p.author_id)
        : p.author_username,
...
export const createPost = async (req, res, next) => {
  try {
    ...
    if (req.isServerKey) {
      if (!authorId)
        return res.status(400).json({
          error: { message: "authorId is required for server-level calls" },
        });
      ...
      if (!req.apiKeyUserId)
        return res
          .status(403)
          .json({ error: { message: "Not authorized to create posts" } });
...
export const updatePost = async (req, res, next) => {
  try {
    ...
    if (
      !req.isServerKey &&
      Number(existing.author_id) !== Number(req.apiKeyUserId)
    ) {
      return res
        .status(403)
        .json({ error: { message: "Cannot modify another user's post" } });
    }

    const post = await Post.update(id, existing.author_id, {
      content,
      imageUrl,
    });
    res.json({ post });
...
export const deletePost = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: { message: "Cannot delete another user's post" } });
    }
...
export const getMockDataset = async (req, res, next) => {
  try {
    const [users, leaderboard, posts] = await Promise.all([
      ...
      Game.getLeaderboard("spit_royale", {
        limit: 10,
        offset: 0,
        publicOnly: true,
      }),
