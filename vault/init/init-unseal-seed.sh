...
#
#   First boot  → initializes Vault, unseals, seeds secrets, writes keys to volume
#   Reboots     → reads unseal key from volume, unseals, re-seeds (idempotent)
...
# ── First-boot: initialize if not yet initialized ────────────────────────────
...
if [ "$IS_INITIALIZED" = "0" ]; then
  ...
  # ── Subsequent boots: unseal using persisted key ──────────────────────────
...
# ── Create or refresh the limited backend service token ───────────────────────
# Runs every time so the token in the keys file is always fresh (90-day TTL).
