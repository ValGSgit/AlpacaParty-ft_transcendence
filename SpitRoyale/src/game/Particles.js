...
export class ParticleSystem {
  ...
  spitImpact(x, z, big = false) {
    ...
    this._burst(x, 0.5, z, count, color, 0.12, 4, 0.8);
  }

  // Shield block
  shieldBlock(x, z) {
    this._burst(x, 0.9, z, 20, 0x4cc9f0, 0.1, 3, 0.6);
    this._ring(x, 0.9, z, 0x4cc9f0);
  }
  ...
  powerupPickup(x, z, type) {
    ...
    this._burst(x, 0.5, z, 30, color, 0.15, 5, 1.2);
    this._ring(x, 0.5, z, color);
  }

  // Dust trail when walking
  dustPuff(x, z) {
    if (Math.random() > 0.3) return;
    this._burst(x + (Math.random()-0.5)*0.4, 0.08, z + (Math.random()-0.5)*0.4, 3, 0xd4c09a, 0.06, 1.2, 0.4);
  }

  // Player eliminated
  elimination(x, z, color) {
    this._burst(x, 1, z, 40, color, 0.2, 8, 1.5);
    this._ring(x, 0.5, z, color);
    for (let i = 0; i < 15; i++) {
      this._burst(
        x + (Math.random()-0.5)*2, 0.5 + Math.random()*2,
        z + (Math.random()-0.5)*2, 4, 0xf4a261, 0.14, 3, 0.8
      );
    }
  }

  _burst(ox, oy, oz, count, color, size, speed, life) {
    ...
    const mat = new THREE.PointsMaterial({
      color, size, sizeAttenuation: true, transparent: true, opacity: 1,
    });
  ...
  _ring(ox, oy, oz, color) {
    ...
    this.scene.add(ring);
    this.emitters.push({ type: 'ring', ring, mat, life: 0.5, maxLife: 0.5, scale: 0.1 });
  }
